...
export default class Thought extends Component {
  ...
  componentDidMount = () => {
    ...
    .then((value) => {
      http('viewThoughts',{},'POST',value)
        .then((response) => {
        if(response.status){
          for(i=0; i<response.thoughtList.length; i++) {
            let date = new Date(response.thoughtList[i].contentDate).toUTCString()
            date = date.split(' ').slice(0, 4).join(' ')
            const content = response.thoughtList[i].thought
            SECTIONS.push({title:date, content:content})
          //this.setState({ dataSource: ds.cloneWithRows(response.thoughtList) })
          }
        }
    ...
    return (
      <View style={styles.container}>      
        {/*<ListView
          ...
          renderRow={(rowData) => <Text style={styles.text}>{rowData.thought}</Text>}
        />*/}
